...
One of the most immediate attacks is **prompt injection**, where malicious input is crafted to override an LLM’s intended instructions. For a deeper dive into how this specific vulnerability can be weaponized into "AI malware," read my article on [PromptLock: The AI Malware](./promptlock-the-ai-malware). This attack is particularly dangerous because it can lead to unauthorized access to sensitive data, such as passwords or API keys, which can be used to gain unauthorized access to systems or networks. The ultimate form of this attack is known as **jailbreaking** – feeding prompts that cause the model to throw out all its safety rules and “disregard its safety protocols entirely”
